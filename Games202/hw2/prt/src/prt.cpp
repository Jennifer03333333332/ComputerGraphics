...
namespace ProjEnv
{
    ...
    //Cubemap 上 每个pixel 代表的矩形，投影到 unit sphere上的面积
    ...
    {
        //1 算好cubemapDirs
        std::vector<Eigen::Vector3f> cubemapDirs;//6张图上，每个像素 的 方向vector (unit的)
        ...
        std::vector<Eigen::Array3f> SHCoeffiecents(SHNum);//用一个数组来存SH系数，
        ...
        for (int i = 0; i < 6; i++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    //访问第i张贴图，(x,y)的方向向量：cubemapDirs[i * width * height + y * width + x] 
                    Eigen::Vector3f dir = cubemapDirs[i * width * height + y * width + x];
                    int index = (y * width + x) * channel;
                    Eigen::Array3f Le(images[i][index + 0], images[i][index + 1],
                                      images[i][index + 2]);//这张图，这个texel的L_env 
                    
                    //Cubemap 上 每个pixel 代表的矩形，投影到 unit sphere上的面积
                    double area_each_texel = CalcArea((float)x, (float)y, width, height);
                    double theta = acos(dir.z());
                    double phi = atan2(dir.y(), dir.x());

                    for(size_t level = 0; level <= SHOrder; level++){
                        for(size_t m = (-1)*level; m <= level; m++){//编号
                            
                            double sh_basic = sh::EvalSH(level, m, phi, theta);//求基函数，封装好了
                            
                            //m,level 到数组index的转换 : 前l阶的基函数个数 + m在当阶是第几个基函数 
                            size_t coeff_index = (level)*(level) + m - (-1)*level;
                            SHCoeffiecents[coeff_index] += Le * sh_basic * area_each_texel;
                        }
                    }
...
{
public:
    static constexpr int SHOrder = 2;//取前SHOrder + 1阶， 从0开始
    static constexpr int SHCoeffLength = (SHOrder + 1) * (SHOrder + 1);//前SHOrder + 1阶 一共有多少个SH_coeff
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        // Projection environment 输出路径
        auto cubePath = getFileResolver()->resolve(m_CubemapPath);//xml里记录的cubemap的path
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)//对这个mesh的每个vertex
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                if (m_Type == Type::Unshadowed)
                {
                    ...
                    //double trans_shcoeff;
                    double cos_term = n.dot(wi);
                    cos_term = (cos_term > 0.0) ? cos_term : 0.0;
                    return cos_term;
                    //double sh_basic = sh::EvalSH(level, m, phi, theta);
                            
                    //将cos term投影到SH求系数
                    // for(int j = 0;j<SHCoeffLength;++j){
                    //     trans_shcoeff += cos_term * 
                    // }
                }
                else
                {
                    ...
                    //cos_term = (cos_term > 0.0) cos_term : 0.0;
                    if( cos_term > 0.0){
                        const Ray3f ray(v, wi);//wi 是在单位球 空间，可以直接对应到世界空间的v
